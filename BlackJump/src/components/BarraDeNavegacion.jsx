    ...
    >
      ...
      <Link to="/" className="flex items-center gap-2 group">
        ...
        <h1 className="text-xs font-serif tracking-[0.4em] text-white uppercase">
          Black
          <span className="italic font-light text-gray-400 font-sans lowercase">
            jump
          </span>
        </h1>
      </Link>
        ...
        {menuItems.map((item, index) => {
          ...
          return (
            ...
            >
              {item.name}
              <span
                className={`absolute bottom-0 left-0 h-[1px] bg-white transition-all duration-300 ${isActive ? "w-full" : "w-0 group-hover:w-full"}`}
              />
            </Link>
          );
        })}
      ...
      <div className="flex items-center gap-6 text-gray-400">
        ...
        <Link
          to="/carrito"
          className="hover:text-white transition-colors relative p-1 group"
        >
        ...
        {isLoggedIn ? (
          ...
          <Link
            to="/login"
            className="hidden md:block text-[9px] uppercase tracking-[0.2em] border border-white/10 px-4 py-2 rounded-lg hover:bg-white hover:text-black hover:border-white transition-all duration-500"
          >
            Iniciar Sesión
          </Link>
        )}
